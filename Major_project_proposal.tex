...
This project provided valuable insights across the vision, speech, and planning modules. Rather than treating each subsystem in isolation, we reflect both on module-specific lessons and broader takeaways that emerged from integrating the full system.

%----------------------------------------
\subsubsection{Reflections on the Vision System}

\paragraph{From Black-Box Implementation to Deeper Understanding}
The most significant technical growth came from moving beyond treating models such as SAM and GraspNet as ``black boxes'' to developing a deeper understanding of their underlying principles. Initially, this limited our ability to troubleshoot effectively. We learned that robust engineering in computer vision requires understanding \textit{why} a model behaves a certain way, not just \textit{how} to use it.

\paragraph{Navigating Model Limitations and Environmental Constraints}
A key takeaway was learning to critically evaluate each model's limitations in real-world conditions. Lighting variations and object geometry had a much greater impact on performance than anticipated. If starting over, we would dedicate more time upfront to testing models under varied environmental conditions, rather than addressing failures retroactively.

%----------------------------------------
\subsubsection{Reflections on the Speech System}

\paragraph{Robustness to User Variability}
One of the key observations from the speech module was the system’s sensitivity to user behavior and speaking style. While the ASR pipeline achieved high accuracy during evaluation, the participants were all expert users who were familiar with the expected command structure. This resulted in consistently well-formed inputs and clear pronunciation. However, this also revealed a limitation: the system’s performance may not generalize as effectively to non-expert users, who may introduce variations in phrasing, pronunciation, or speaking pace. This highlights the importance of evaluating ASR systems under more diverse and realistic conditions, and suggests that robustness to user variability remains an important area for improvement.

\paragraph{Trade-off Between Safety and Natural Interaction}
Another important insight was the trade-off introduced by the keyword-based command gating mechanism. The requirement for a strict activation keyword (e.g., \texttt{``EXECUTE''}) ensures that only intentional commands are forwarded to the planning system, significantly reducing the risk of unintended robot actions. Similarly, the use of dedicated emergency and recovery keywords (\texttt{``STOP''}, \texttt{``OKAY''}) provides a simple and reliable safety interface. However, this design constrains the naturalness of interaction, requiring users to conform to a rigid command structure. This demonstrates that while keyword-based control is effective for safety-critical systems, it limits flexibility and highlights the need for more advanced intent recognition mechanisms in future work.


%----------------------------------------
\subsubsection{Reflections on the Planning System}

\paragraph{The Importance of Structured Intermediate Representations}
One of the most significant insights from the planning module was the impact of introducing a structured intermediate representation (IR) between natural language and PDDL generation. While directly prompting the LLM to generate PDDL was conceptually simple, it often resulted in syntactic errors and invalid problem definitions. By constraining the output space through an IR, we were able to significantly improve reliability and consistency. This highlighted that, for complex symbolic tasks, LLMs benefit from operating within well-defined structures rather than being relied upon for fully end-to-end generation.

\paragraph{Understanding and Differentiating Failure Modes}
Another key lesson was recognizing that different planning approaches fail in fundamentally different ways. The LLM-direct approach often produced plans that appeared reasonable but failed during execution due to mismatches with the physical system. In contrast, direct PDDL generation frequently failed earlier due to syntax and parsing errors. The IR-based approach reduced both types of failures but did not eliminate them entirely. This emphasized the importance of not only evaluating success rates, but also analyzing \textit{how} and \textit{why} failures occur. Such understanding is crucial for designing more robust systems and informed our decision to prioritize structured generation and validation mechanisms.

%----------------------------------------
\subsubsection{Overall Reflections}

\paragraph{From Implementation to Research-Oriented Thinking}
Beyond individual modules, the project highlighted the importance of moving from implementation-focused development to a more research-oriented mindset. This included formulating hypotheses, designing structured experiments, and systematically evaluating results. We found that careful documentation and iterative refinement were as critical to success as the system implementation itself.

\paragraph{System Integration as a Core Challenge}
A major insight from this project was that integrating vision, speech, and planning is fundamentally more challenging than developing each module in isolation. Small errors in perception or language understanding often propagated downstream, leading to planning failures or incorrect execution. This highlighted the importance of well-defined interfaces and robust intermediate representations between modules. We learned that system design must account not only for individual component performance, but also for how uncertainty and errors accumulate across the pipeline.

\paragraph{Iterative Development and Evaluation}
Another key takeaway was the importance of structured and early evaluation. In several cases, issues in the system were only identified after full pipeline integration, making them more difficult to diagnose. If starting over, we would adopt a more incremental evaluation strategy, benchmarking each module under controlled conditions before integration. Additionally, defining clear evaluation metrics earlier in the project would have enabled more systematic comparison between different approaches and faster identification of bottlenecks.

\paragraph{What We Would Do Differently}
If we were to start the project again, we would place greater emphasis on system-level design rather than focusing primarily on implementation. In particular, we would define clearer goals, key performance indicators (KPIs), and evaluation metrics early in the project. Establishing these criteria upfront would allow us to more effectively benchmark progress and guide design decisions toward measurable outcomes.

Additionally, we would adopt a more iterative development process by testing early and frequently. Earlier and more frequent testing at both the module and system levels would enable faster identification of bottlenecks, allowing us to refine the design and adapt more efficiently throughout the development process.
...
Overall system performance was evaluated through end-to-end benchmarking across six representative manipulation scenarios, using the KPI defined at the end of the \textit{Evaluation Metrics and Benchmarks} subsection in Section~\ref{sec: objectives}: success rate over 30 trials, completion time within 15 seconds, and minimal disturbance of non-target objects below 2~cm. Each scenario was executed using four settings: \textbf{LLM-Direct}, \textbf{PDDL-Direct}, \textbf{IR-Based PDDL}, and \textbf{Manual} execution. For each automated approach, every scenario was repeated five times, and planning and execution times were recorded separately. During success-rate evaluation, a trial was counted as a failure if the task disturbed any non-target object by more than 2~cm. For the manual setting, each scenario was performed by five participants (three experts familiar with the system and two non-experts). Non-experts required slightly longer time on average, but the difference was not statistically significant. For manual execution, only total completion time is reported because planning and execution cannot be cleanly separated in human operation.

The six scenarios were designed to cover distinct system capabilities:
\begin{enumerate}
    \item Basic handover of a single object in a crowded space.
    \item Handover of a specific object within a duplicate-object group.
    \item Clarification behavior under ambiguous user instructions.
    \item Identification and handover based on positional reference.
    \item Identification and handover based on color reference.
    \item Placement of an object at a specified position relative to another object.
\end{enumerate}

\begin{table}[h]
\centering
\small
\caption{Planning time (seconds) across scenarios.}
\label{tab:overall-planning-time}
\begin{tabular}{lccc}
\toprule
\textbf{Scenario} & \textbf{LLM-Direct} & \textbf{PDDL-Direct} & \textbf{IR-Based PDDL} \\
\midrule
1 & 3.642 & 4.135 & 4.206 \\
2 & 7.500 & 9.410 & 8.396 \\
3 & 5.274 & 6.6725 & 5.854 \\
4 & 11.4775 & 13.5367 & 11.818 \\
5 & 5.104 & 6.042 & 5.272 \\
6 & 5.780 & 6.460 & 6.142 \\
\midrule
\textbf{Average} & \textbf{6.460} & \textbf{7.710} & \textbf{6.950} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Execution time (seconds) across scenarios.}
\label{tab:overall-execution-time}
\begin{tabular}{lcccc}
\toprule
\textbf{Scenario} & \textbf{LLM-Direct} & \textbf{PDDL-Direct} & \textbf{IR-Based PDDL} & \textbf{Manual (Total)} \\
\midrule
1 & 40.947 & 37.265 & 36.181 & 1.688 \\
2 & 47.794 & 43.308 & 42.735 & 1.522 \\
3 & 35.239 & 31.975 & 32.095 & 3.608 \\
4 & 42.834 & 37.332 & 37.619 & 1.706 \\
5 & 43.483 & 40.074 & 38.143 & 1.204 \\
6 & 62.605 & 57.028 & 54.623 & 1.438 \\
\midrule
\textbf{Average} & \textbf{45.484} & \textbf{41.180} & \textbf{40.232} & \textbf{1.860} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Success rate (\%) across scenarios.}
\label{tab:overall-success-rate}
\begin{tabular}{lcccc}
\toprule
\textbf{Scenario} & \textbf{LLM-Direct} & \textbf{PDDL-Direct} & \textbf{IR-Based PDDL} & \textbf{Manual} \\
\midrule
1 & 100 & 80 & 100 & 100 \\
2 & 60 & 60 & 100 & 100 \\
3 & 100 & 80 & 100 & 100 \\
...
6 & 40 & 40 & 100 & 100 \\
\midrule
\textbf{Average} & \textbf{80} & \textbf{70} & \textbf{100} & \textbf{100} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Combined averages from planning time, execution time, and success rate (green = best, red = worst).}
\label{tab:overall-combined-averages}
\begin{tabular}{lcccc}
\toprule
\textbf{Metric} & \textbf{LLM-Direct} & \textbf{PDDL-Direct} & \textbf{IR-Based PDDL} & \textbf{Manual} \\
\midrule
Planning Time (s) & \textcolor{green!50!black}{\textbf{6.460}} & \textcolor{red!70!black}{\textbf{7.710}} & 6.950 & N/A \\
Execution Time (s) & \textcolor{red!70!black!}{\textbf45.484} & 41.180 & 40.232 & \textcolor{green!50!black}{\textbf{1.860}} \\
Success Rate (\%) & 80 & \textcolor{red!70!black}{\textbf{70}} & \textcolor{green!50!black}{\textbf{100}} & \textcolor{green!50!black}{\textbf{100}} \\
\bottomrule
\end{tabular}
\end{table}

Tables~\ref{tab:overall-planning-time}--\ref{tab:overall-success-rate} summarize system performance across planning time, execution time, and success rate, respectively. Table~\ref{tab:overall-combined-averages} consolidates the average values from these three tables for quick comparison.

The results indicate that \textbf{IR-Based PDDL} is the most reliable automated approach, achieving a perfect success rate (100\%) across all scenarios and meeting the KPI target of at least 90\% success over 30 trials. In contrast, LLM-Direct and PDDL-Direct are less consistent, with average success rates of 80\% and 70\%, respectively. Both approaches degrade substantially in Scenario 6, where each achieves only 40\% success, suggesting that the structured intermediate representation improves robustness in more constrained or failure-prone cases.

Importantly, the failure modes of these two approaches differ. For \textbf{LLM-Direct}, failures primarily occur during physical execution, where the action sequences generated by the model cannot be reliably executed by the robot. These errors arise despite producing plausible high-level plans, indicating a mismatch between generated actions and the robot's execution constraints. In contrast, failures in \textbf{PDDL-Direct} are more often due to syntax or parsing errors in the generated PDDL, which prevent successful planning altogether. This distinction highlights that LLM-Direct suffers from execution-level unreliability, whereas PDDL-Direct is more sensitive to formal correctness.

In terms of efficiency, LLM-Direct has the lowest average planning time among automated methods (6.460\,s), followed by IR-Based PDDL (6.950\,s) and PDDL-Direct (7.710\,s), as shown in Table~\ref{tab:overall-planning-time}. However, we were not able to achieve the targeted completion time of 15 seconds. The slow end-to-end execution is dominated by the vision pipeline, especially because models such as SAM, CLIP, and the VLM are all run locally, which adds substantial inference latency. Execution time remains similar across all automated methods (approximately 40--45\,s), as shown in Table~\ref{tab:overall-execution-time}, indicating that perception and physical robot execution dominate end-to-end latency more than planner computation. The additional planning overhead of IR-Based PDDL relative to LLM-Direct is modest (approximately 0.49\,s) while providing a substantial reliability gain.

Compared with automated pipelines, manual operation is significantly faster, with an average total time of 1.860\,s and 100\% success. This corresponds to roughly a 30$\times$ speed gap compared to automated execution time. Overall, the benchmark highlights a clear trade-off: IR-Based PDDL offers the best reliability for autonomous operation, while future improvements should prioritize reducing execution latency to close the gap with human-level efficiency.
...
To evaluate the reliability of the perception stack during hardware deployment, we benchmarked the core vision services under varying environmental illumination. The protocol comprised five unique objects scattered randomly in the workspace across four controlled lighting intensities (250, 200, 70, and 15 Lux), yielding 50 independent trials per intensity (200 total hardware trials). 

Prior to analyzing the results, we define the benchmark metrics utilized:
\begin{itemize}
    \item \textbf{SAM IoU \& Stability Rate:} Intersection over Union (IoU) is computed following standard COCO evaluation metrics \cite{lin2014microsoft}. Stability Rate is defined as the pass rate for predictions achieving an Average Precision (AP) $\ge 0.5$.
    \item \textbf{CLIP Top-1 Accuracy:} The average success rate of the model correctly assigning the ground-truth semantic label to a SAM-cropped region.
    \item \textbf{Localization Confidence:} For the \texttt{find\_object} service, confidence is calculated via the cosine similarity between the normalized CLIP text embedding of the query ($\mathbf{f}_{\text{text}}$) and the image embedding of the detected region ($\mathbf{f}_{\text{image}}$):
    $$ \text{Similarity} = \left( \frac{\mathbf{f}_{\text{image}}}{\|\mathbf{f}_{\text{image}}\|} \right) \cdot \left( \frac{\mathbf{f}_{\text{text}}}{\|\mathbf{f}_{\text{text}}\|} \right)^T $$
    Similarity scores range from $-1.0$ to $1.0$, with a strict rejection threshold enforced at $< 0.2$.
    \item \textbf{HITL Accuracy:} Human-In-The-Loop (HITL) empirical verification of the predicted bounding box correctness.
\end{itemize}

\subsubsection*{Illumination Robustness and Mask Stability (SAM)}
As detailed in Table~\ref{tab:sam_benchmark}, SAM maintains high reliability at or above 70 Lux, consistently achieving an IoU $> 0.80$ with stable latency. Performance peaks at 200 Lux but degrades critically at 15 Lux. In near-darkness, the Stability Rate collapses to $0.26$, establishing 70 Lux as the minimum workspace illumination threshold for reliable semantic grasping.

\begin{table}[htbp]
    \centering
    \caption{SAM benchmark across varying lighting conditions. Bold indicates the best value per metric; $\dagger$ marks the minimum acceptable illumination threshold (70\,Lux).}
    \label{tab:sam_benchmark}
    \small
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lS[table-format=1.3]S[table-format=1.3]S[table-format=1.2]S[table-format=1.2]}
        \toprule
        \textbf{Intensity (Lux)} & {\textbf{Avg Confidence}} & {\textbf{Avg IoU}} & {\textbf{Stability Rate}} & {\textbf{Latency (s)}} \\
        & & & {(AP $\ge$ 0.5)} & \\
        \midrule
        250         & 0.649          & 0.906          & 0.96          & \textbf{0.50} \\
        200         & \textbf{0.655} & \textbf{0.925} & \textbf{0.96} & \textbf{0.50} \\
        70$^\dagger$ & 0.648         & 0.820          & 0.86          & \textbf{0.50} \\
        15          & 0.601          & 0.371          & 0.26          & 0.60          \\
        \bottomrule
    \end{tabular*}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{images/SAM_70_LUX.png}
\caption{SAM Performance at 70 Lux}
\label{fig:sam-70-lux}
\end{figure}


\subsubsection*{Zero-Shot Semantic Classification (CLIP)}
CLIP demonstrated notable resilience to lighting variations (Table~\ref{tab:clip_benchmark}). While accuracy peaked at 200 Lux ($0.826$), a slight performance drop at 250 Lux suggests surface overexposure may hinder feature extraction. Even at 15 Lux, CLIP maintained a Top-1 Accuracy of $0.729$, indicating that given a valid bounding box from SAM, CLIP's embedding space remains robust in low-light conditions with negligible latency overhead.

\begin{table}[htbp]
    \centering
    \caption{CLIP Benchmark across varying lighting conditions.}
    \label{tab:clip_benchmark}
    \begin{tabular}{lcc}
        \toprule
        \textbf{Intensity (Lux)} & \textbf{Top-1 Accuracy} & \textbf{Latency (s)} \\
        \midrule
        250 & 0.873 & 0.032 \\
        200 & 0.826 & 0.031 \\
        70  & 0.647 & 0.031 \\
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{images/CLIP_70_LUX.png}
\caption{Zero-Shot Semantic Classification Performance at 70 Lux}
\label{fig:clip-70-lux}
\end{figure}


\subsubsection*{Oriented Bounding Box (OBB) Localization}
To evaluate the deterministic planar grasping pipeline, we analyzed the \texttt{find\_object} service. For rotational alignment, the system achieved an average SkewIoU of $0.149$. While numerically lower than standard axis-aligned IoU, it reflects the strict penalty SkewIoU applies to angular misalignments, underscoring the hyper-sensitivity of planar grasping to spatial distortions and justifying the implementation of our hybrid intrinsic-empirical calibration model. 

Crucially, across 14 complex edge-case scenarios, the system achieved an HITL Accuracy of $0.786$, despite reporting an average internal confidence (cosine similarity) of only $0.276$. This discrepancy highlights that while the model's similarity calibration approaches the functional rejection limit ($0.2$) in cluttered real-world settings, its geometric predictions remain functionally accurate for task execution.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{images/OBB_200_LUX.png}
\caption{Oriented Bounding Box Performance at 200 Lux}
\label{fig:obb-200-lux}
\end{figure}


\subsection{Speech}

The ASR system was evaluated using a set of 30 task commands of varying complexity. Three participants (Kanisorn, Methasit, and Withawin), all of whom are expert users familiar with the system and command structure, each performed all tasks. System performance was measured in terms of successful recognition and correct transmission of the command to the planning node.
...
Planning performance was evaluated on Blocksworld tasks using the Blocksworld Image Reasoning Dataset (BIRD)~\cite{bird}. From this dataset, we randomly sampled 155 scenarios for evaluation. Each task consists of an initial state (provided as an image) and a natural-language goal. A vision-language model first converts the image into a textual state description, which is concatenated with the goal instruction and passed to the planning pipeline.

All three approaches were evaluated under the same experimental setup to ensure a fair comparison. The language model used for generation in all settings is \texttt{gpt-oss:20b}, and the vision-language model used for image-to-text conversion is \texttt{qwen3-vl:8b}. Both models are deployed locally using Ollama. For the JSON and PDDL settings, the generated problem is solved using Fast Downward with a fixed domain file~\cite{the-fast-downward-planning-system}.

Figure~\ref{fig:planning-benchmark-flow} shows the benchmarking flow used to compare the three planning settings: direct LLM generation, direct PDDL generation, and IR-based generation.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/pddl_generation_benchmark.png}
    \caption{Benchmarking flow for planning approaches: LLM-direct, direct PDDL generation, and IR-based generation.}
    \label{fig:planning-benchmark-flow}
\end{figure}

We evaluate three main aspects of planning performance. \textit{Planning Success Rate} measures whether the generated representation leads to a valid plan that achieves the goal. \textit{Syntactic Validity} indicates whether the generated PDDL can be successfully parsed by the planner. \textit{Efficiency} is measured through token usage and runtime, where token counts approximate generation cost and elapsed time captures end-to-end latency. For symbolic approaches, we additionally evaluate \textit{Symbolic Accuracy} using precision, recall, and the balanced F-score $F_{pd1}$:
\[
F_{pd1}=\frac{2PR}{P+R}
\]
where precision $P$ measures the correctness of generated symbols and recall $R$ measures coverage of ground-truth symbols~\cite{planowl}.

\begin{table}[H]
\centering
\small
\caption{Planning success on BIRD (155 sampled scenarios).}
\label{tab:planning-performance}
\begin{tabular}{lcc}
\toprule
\textbf{Approach} & \textbf{Plan Found (\%)} & \textbf{Syntactically Valid (\%)} \\
\midrule
JSON & \textbf{\textcolor{green}{65.8}} & \textbf{\textcolor{green}{97.1}} \\
PDDL & 45.2 & \textbf{\textcolor{red}{93.5}} \\
LLM & \textbf{\textcolor{red}{39.4}} & N/A \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Efficiency comparison on BIRD (155 sampled scenarios).}
\label{tab:planning-efficiency}
\begin{tabular}{lccc}
\toprule
\textbf{Approach} & \textbf{Input Tokens} & \textbf{Output Tokens} & \textbf{Mean Time (s)} \\
\midrule
JSON & \textbf{\textcolor{green}{624.89}} & 51.23 & 54.30 \\
PDDL & \textbf{\textcolor{red}{689.70}} & \textbf{\textcolor{red}{411.46}} & \textbf{\textcolor{red}{54.97}} \\
LLM & 646.96 & \textbf{\textcolor{green}{31.12}} & \textbf{\textcolor{green}{44.39}} \\
\bottomrule
\end{tabular}
\end{table}
...
\label{tab:planning-f1-results}
\begin{tabular}{lccc}
\toprule
\textbf{Approach} & \textbf{Precision} & \textbf{Recall} & \textbf{F1} \\
\midrule
PDDL & \textbf{\textcolor{red}{0.3730}} & \textbf{\textcolor{red}{0.3837}} & \textbf{\textcolor{red}{0.3756}} \\
JSON & \textbf{\textcolor{green}{0.5784}} & \textbf{\textcolor{green}{0.5765}} & \textbf{\textcolor{green}{0.5732}} \\
\bottomrule
\end{tabular}
\end{table}

The results show that the JSON-based (IR) approach achieves the strongest overall performance across all evaluated dimensions. It attains the highest planning success rate (65.8\%), substantially outperforming both direct PDDL generation (45.2\%) and LLM-based plan generation (39.4\%). It also achieves the highest symbolic accuracy, with an $F_{pd1}$ score of 0.5732.

In terms of efficiency, the LLM-based approach is the fastest and most lightweight, with the lowest mean runtime (44.39\,s) and output token usage (31.12 tokens). However, this efficiency comes at the cost of reduced planning success and the absence of explicit symbolic representations, making direct comparison in terms of syntactic validity and symbolic accuracy not applicable.

Comparing the two symbolic approaches, IR-based generation consistently outperforms direct PDDL generation. It produces significantly fewer output tokens (51.23 vs. 411.46), indicating that the model avoids generating verbose formal syntax and instead focuses on structured semantic content. Despite this reduction, runtime remains comparable, suggesting that improvements are driven primarily by output quality rather than computational speed.

Both structured approaches maintain high syntactic validity, with JSON at 97.1\% and PDDL at 93.5\%. This indicates that LLMs are generally capable of producing well-formed structured outputs. However, the higher planning success and symbolic accuracy of the IR-based approach suggest that explicitly separating semantic representation from syntactic realization leads to more reliable downstream planning.
...
In the earlier implementation, scene understanding inferred spatial relationships primarily from detected bounding boxes and object centroids. This was brittle for complex interactions, fine-grained occlusions, and non-standard arrangements, and could produce incomplete relational outputs. The updated implementation using \texttt{/vision/vqa} with a VLM improves this limitation by enabling more flexible, open-ended interpretation of scene context from natural-language queries over live RGB input.

However, this new approach introduces its own limitations. Because scene interpretation now relies on a VLM rather than a fully deterministic geometric algorithm, outputs can vary across prompts and may occasionally be inconsistent in edge cases. In addition, VLM-based inference is computationally heavier than deterministic centroid- or rule-based methods, increasing resource usage and potentially affecting response latency on constrained hardware. This creates a practical tradeoff: better semantic flexibility and context awareness versus higher compute demand and reduced determinism.

Furthermore, the pipeline does not yet incorporate temporal consistency, meaning rapid movements or real-time dynamic changes in the scene can still produce unstable predictions. 
...
For future development, these limitations could be addressed by incorporating multi-view or higher-resolution sensors, improving depth filtering and denoising, extending GraspNet and scene understanding models to handle irregular objects, and integrating temporal smoothing or tracking to enhance robustness in dynamic real-world scenarios. For the VQA component specifically, future work should combine VLM outputs with deterministic geometric checks to improve consistency, and explore model optimization or lightweight deployment strategies to reduce computational overhead. These improvements aim to increase perception reliability, grasp success, and overall system usability in complex and unstructured environments.

\subsection{Speech}

Despite the successful integration of the ASR module, several limitations remain. Currently, the system supports only one deactivation keyword, ``EXECUTE,'' which must appear at the end of a spoken command for it to be transmitted to the planning node. Commands that do not include this keyword are ignored, which reduces flexibility and limits the naturalness of user interaction. Similarly, the emergency stop mechanism recognizes only the keyword ``STOP'' to immediately halt robot operation, and can only be reset using the word ``OKAY.'' While these constraints improve reliability and ensure safe operation, they restrict the diversity of acceptable voice inputs.

Another limitation is the absence of an integrated text-to-speech (TTS) module in the deployed ROS2 pipeline. Without verbal system feedback, interaction is less natural and less conversational, and users receive reduced immediate confirmation about command reception and task status.

Although the newly introduced chat GUI partially mitigates this issue by providing visible feedback and explicit state transitions, it still cannot fully replicate the naturalness of bidirectional voice interaction.

In addition, the current evaluation of the ASR system was conducted using expert users who are familiar with the command structure and system behavior. This introduces a bias in the reported performance, as real-world users may exhibit greater variability in speech patterns, phrasing, and pronunciation. As a result, the measured accuracy may overestimate the system's performance in more general deployment scenarios.

For future development, the ASR system will be extended to support more flexible and natural language interaction, including multiple activation and termination phrases. A feedback mechanism will also be introduced, enabling the robot to verbally confirm received commands and notify users upon task completion. This interactive feedback loop is expected to improve user awareness, communication transparency, and overall usability in real-world human--robot collaboration.
\subsection{Planning}
Despite the successful implementation of the planning pipeline, several limitations remain. The most significant challenge lies in generating semantically correct and task-consistent PDDL problem files from natural language and perception outputs. In the current pipeline, the \texttt{domain.pddl} file is fixed and manually authored, while the \texttt{problem.pddl} file is produced through an IR-to-template conversion flow. This design largely prevents syntax failures, but errors can still occur at the semantic level, such as missing objects, incomplete initial predicates, or incorrect goal conditions.

The results also show that the intermediate representation is manually designed, which requires domain knowledge to define an appropriate schema. Although the structured JSON format improves reliability and symbolic accuracy, it is still tailored to the current task family and does not yet capture the diversity of more complex planning settings. In addition, the evaluation in this project is limited to the Blocksworld domain, which remains relatively constrained.

Future work should therefore expand beyond Blocksworld to richer manipulation domains with larger predicate sets, partial observability, and longer-horizon tasks. A particularly useful direction is to compare alternative intermediate representations and output formats, including TOON, Markdown, XML, and related structured encodings, to measure their relative efficiency in token usage and output accuracy. Beyond representation choice, future studies should investigate output evaluation and feedback loops, stronger prompt engineering, and multi-agent approaches to improve robustness, controllability, and downstream planning quality.
...
Despite establishing a stable and reproducible hardware execution environment, several limitations still exist. In particular, motion from the UR7e introduces vibrations in the camera mount, which can reduce the consistency of visual pose estimation during operation. These vibrations affect repeatability and may introduce minor calibration drift over time.

Additionally, the acrylic plate positioned on the robot base is used as a controlled visual background within the camera's field of view. However, its current implementation does not fully eliminate background noise under all lighting conditions, which can impact perception robustness. Variations in illumination and reflections on the acrylic surface may still introduce inconsistencies in object detection and segmentation.

These limitations highlight the need for improved mechanical stability and more controlled visual conditions to ensure reliable real-world, perception-driven performance.
