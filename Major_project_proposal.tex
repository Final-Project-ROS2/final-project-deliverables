...
\begin{table}[htbp]
    \centering
    \caption{RMSE comparison of pure intrinsic versus hybrid intrinsic-empirical models at two camera heights.}
    \label{tab:error_isolation}
    \begin{tabular}{@{}p{2.5cm}p{3cm}p{3.5cm}p{6cm}@{}}
        \toprule
        \textbf{Camera Height} & \textbf{Intrinsic Only (RMSE)} & \textbf{Intrinsic + Empirical (RMSE)} & \textbf{Observation} \\
        \midrule
        $0.67\,\text{m}$ & $1.28\,\text{cm}$ & $1.56\,\text{cm}$ & Intrinsic model performs best; empirical model overfits and adds noise at this height. \\
        $0.55\,\text{m}$ & $2.35\,\text{cm}$ & $1.35\,\text{cm}$ & Empirical correction successfully mitigates systematic distortion at the lower standoff. \\
        \bottomrule
    \end{tabular}
\end{table}
