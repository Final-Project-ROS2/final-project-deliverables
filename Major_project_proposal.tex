...
The results demonstrate that the ASR module achieves a high recognition success rate across participants, with an average performance of approximately 97.9\%. Simple commands were consistently recognized with 100\% success, while a few complex multi-step instructions occasionally resulted in a single misrecognition. Emergency stop and deactivation word features were correctly detected in all corresponding instances.

Overall, these results confirm that the ASR system reliably interprets natural language instructions, accurately transmits commands to the planning node, and supports safe human–robot interaction for real-time manipulation tasks.
