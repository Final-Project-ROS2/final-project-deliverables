...
The vision subsystem successfully integrated SAM, CLIP, and GraspNet into a unified pipeline to guide the robot's manipulation tasks. Beyond implementation, the project provided valuable insights into applied computer vision and research methodology. Our key reflections fall into three areas:

\paragraph{From Black-Box Implementation to Mathematical Understanding}
The most significant technical growth came from moving beyond model implementation to understanding the mathematics behind it. Treating SAM and GraspNet as ``black boxes'' initially limited our ability to troubleshoot effectively. We learned that robust engineering in computer vision requires understanding \textit{why} a model behaves a certain way, not just \textit{how} to call its functions.

\paragraph{Navigating Model Limitations and Environmental Constraints}
A key takeaway was learning to critically evaluate each model's limitations against real-world constraints. Lighting variations and object geometry had a much greater impact on accuracy than anticipated. If starting over, we would dedicate more time upfront to testing models under varied environmental conditions, rather than addressing failures retroactively.

\paragraph{Mastering the Research Methodology}
Perhaps the most valuable outcome was experiencing the full lifecycle of academic research from formulating a hypothesis and designing experiments, to writing and submitting a paper. We learned that systematic documentation and structured iteration are just as critical to a project's success as the code itself.
