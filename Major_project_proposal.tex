...
Recent work on the vision module focused on improving scene understanding and handling multiple instances of the same object. Two new ROS2 services were added.
...
Progress on the speech module has remained steady but minimal. We have finalized the microphone selection for the project and secured the hardware, and initial development of a text-to-speech (TTS) pipeline is underway to support richer user interaction. Because the core speech recognition functionality reached high accuracy and low latency during semester 1 testing, the team is prioritizing the broader transition from simulation to hardware over additional speech refinements for now.
...
Due to equipment availability and workspace requirements, we transitioned from the UR3e to a newly acquired UR7e. The UR7e provides longer reach and a larger effective workspace, which better matches our current depth-camera placement and planned manipulation area.

This migration required mainly controller-side configuration rather than major software redesign. Our low-level control pipeline continues to use \textit{URScript}, the official Universal Robots scripting language, with commands sent directly to \textit{URControl} over TCP/IP socket communication~\cite{urscript-docs}. At the command-interface level, this approach is largely robot-model independent, so the software stack remained unchanged. The only required software adjustment was redefining predefined robot poses and limits to account for dimensional and kinematic differences between the UR3e and UR7e.

On the teach pendant, we enabled Remote Control and the required services for TCP communication, and verified that the relevant ports were allowed in the security settings. For the gripper, both arms use the same Robotiq Hand-E and are controlled through the same socket-based interface, so gripper integration remained unchanged and the transition was seamless.
...
The current progress is on schedule. Since the primary focus has been on integrating the depth camera with real-world perception, the overall vision pipeline has become more stable while initial calibration and pixel-to-real-world transformation have been successfully implemented. The system is capable of producing consistent 3D coordinate outputs, although minor inaccuracies remain due to calibration sensitivity and depth noise. Ongoing experimentation and iterative testing aim to refine camera parameters, improve transformation reliability, and reduce measurement error. Development is progressing toward a fully calibrated perception module that will be ready for quantitative benchmarking of the vision component once calibration is finalized.

\subsubsection{Speech Processing}

Progress is slightly behind the planned schedule. The team has procured the required microphone and supporting hardware, but the text-to-speech development is moving slower than expected while effort is directed toward migrating the overall system from simulation to physical hardware.
...
Progress is largely on schedule. The depth camera has been securely mounted at the base of the UR arm and is fully operational. Its spatial position relative to the UR base has been measured, and a consistent coordinate reference frame has been established to support camera-to-robot transformation. The experimental workspace has also been prepared using a controlled visual background to improve perception reliability. 

Some minor refinements remain in the mechanical integration. Future improvements include the design and fabrication of custom 3D-printed alignment clamps to enhance mounting stability and repeatability. Additionally, a structured cable management solution and dedicated wire holder are planned to ensure that USB routing does not interfere with the UR arm's range of motion. These enhancements will further improve system robustness and operational safety.
...
\newpage
\section{Limitations and Potential Future Improvements}
