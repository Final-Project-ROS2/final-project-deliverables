...
To improve reliability, the PDDL generation process was redesigned to use an intermediate representation (IR). Instead of generating raw PDDL code, the LLM produces a structured JSON representation describing the task in terms of objects, locations, and goal predicates. This representation captures only the desired final state, allowing the symbolic planner to infer the sequence of actions required to achieve it.
...
As illustrated in Figure~\ref{fig:pddl-pipeline}, the structured IR is deterministically transformed into a valid PDDL problem file using a templating engine (Jinja2). The domain file remains fixed and manually defined, ensuring strict control over available predicates and actions. This two-stage pipeline eliminates syntax errors from LLM-generated code, reduces token usage, and improves interpretability by separating task specification from formal encoding.
