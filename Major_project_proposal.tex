...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/Sytem_Architecture_Overview.png}
...
\begin{itemize}
    ...
    \item \textbf{Symbolic AI}: Explicit reasoning over tasks with preconditions and effects. Constraints enforcement to ensure executability.
...
\begin{enumerate}
    ...
    \item Semantics and discrete concepts are extracted from the text commands using Natural Language Understanding (NLP) techniques, resulting in a latent representation (for symbolic processing) and a detailed description (for LLMs).
    \item The current state of the work area is perceived through a camera.
    \item The live visual feed is processed by applying object detection, segmentation, grasp synthesis, and visual understanding, resulting in a latent representation (for symbolic processing) and a detailed description (for LLMs).
    \item The main algorithm plans and generates the program for the robot arm to execute. This could involve multiple techniques and technologies, such as the use of LLM, Multi-agent system, and PDDL, depending on the latent representation and architecture.
    \item The generated program is analyzed symbolically to ensure executability.
    ...
    \item Once a program is approved by the user, the robot arm executes the program.
