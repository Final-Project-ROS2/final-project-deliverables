...
To establish a robust Automatic Speech Recognition (ASR) foundation for robot manipulation, a real-time speech-to-text system has been fully developed and implemented as a ROS2 node. Unlike the initial prototype, which processed pre-recorded audio inputs such as \textit{harvard.wav}, the current system continuously captures live audio streams, transcribes them in real time, and publishes the recognized text for downstream processing.

AssemblyAI serves as the core transcription engine, leveraging advanced \textbf{deep learning models} to convert speech into accurate text through a structured pipeline that includes \textit{feature extraction}, \textit{acoustic modeling}, \textit{language modeling}, and \textit{decoding}~\cite{assemblyai_docs}. These stages enable the system to handle diverse speech patterns and environmental noise effectively. Furthermore, post-processing mechanisms such as punctuation restoration, capitalization, and disfluency removal enhance the clarity and readability of the generated text.

As illustrated in Figure~\ref{fig:ASR_Pipeline}, the ASR node operates through a continuous loop of capturing live microphone input, sending it as a transcription job, polling for recognition results, and checking for further user input. The system introduces a \textbf{deactivation word mechanism}, where commands are sent to the planning node only if the spoken sentence ends with the word ``execute,'' and an \textbf{emergency stop feature}, which immediately halts the robot when the word ``STOP'' is spoken and can be deactivated using the word ``OKAY.'' These features replace the previous time-based three-second silence detection, enabling more responsive and controlled interactions.

This workflow provides a modular and scalable interface that allows for safe, responsive, and context-aware communication between humans and robots. The ASR module has been successfully integrated with the planning module, establishing a foundation for future extensions, such as verbal feedback confirmation and task completion notifications, to enhance human–robot collaboration.


\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{images/asrpipe.png}
...
% starting visual ros2
\subsubsection*{ROS2 Service-Oriented Integration}

With the individual perception modules completed, the system was refactored into a modular ROS2 service-oriented architecture. All earlier standalone Python modules were migrated into \texttt{rclpy}-based nodes, with image conversion handled through \texttt{cv\_bridge}. This design allows each perception capability—segmentation, classification, grasp estimation, metric conversion, and scene reasoning—to be invoked on demand while maintaining real-time compatibility within robotic control loops.

\paragraph{Node Overview}
The perception stack consists of five ROS2 nodes. SAM provides the initial segmentation, CLIP performs semantic understanding, GraspNet computes grasp poses, the scene-understanding module derives relational context, and the pixel-to-real module converts pixel coordinates into world coordinates using depth images.

\begin{table}[H]
\centering
\caption{ROS2 perception nodes and their core functionalities.}
\begin{tabular}{|l|l|l|}
    \hline
    \thead{\textbf{Node}} & \thead{\textbf{Role}} & \thead{\textbf{Key Topics / Services}} \\
    \hline
    simple\_sam\_detector &  \makecell{Object segmentation}  & \makecell{\texttt{/vision/run\_pipeline}\\\texttt{/vision/detect\_objects}\\\texttt{/vision/show\_depth\_image}\\publishes \texttt{/vision/sam\_detections}} \\
    \hline
    clip\_classifier &  \makecell{Zero-shot semantic\\classification}  & \makecell{\texttt{/vision/classify\_all}\\\texttt{/vision/classify\_bb}\\\texttt{/vision/find\_obj}\\subscribes to \texttt{/vision/sam\_detections}} \\
    \hline
    graspnet\_detector &  \makecell{6-DoF grasp\\pose estimation}  & \makecell{\texttt{/vision/detect\_grasp}\\\texttt{/vision/detect\_grasp\_bb}\\subscribes to \texttt{/vision/sam\_detections}} \\
    \hline
    scene\_understanding &  \makecell{Relational reasoning}  & \makecell{\texttt{/vision/understand\_scene}\\publishes \texttt{/vision/scene\_understanding}\\subscribes to \texttt{/vision/sam\_detections}} \\
    \hline
    pixel\_to\_real &  \makecell{Pixel-to-world\\coordinate conversion}  & \makecell{Subscribes to \texttt{/camera/depth/image\_raw}\\service: \texttt{/vision/pixel\_to\_real}} \\
    \hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=1.2\linewidth]{images/vision_service2.png}
\caption{The complete vision pipeline from perception to reasoning. Demonstrating the progressive transformation of raw input data structured semantic understanding.}
\label{fig: vision_service2}
\end{figure}

\paragraph{Node Descriptions}

\textbf{SAM Detector Node (\texttt{simple\_sam\_detector})} \\
Segments objects using the Segment Anything Model (SAM) and publishes bounding boxes, masks, and distance estimates.

\begin{itemize}
    \item \texttt{/vision/run\_pipeline}: Initiates continuous segmentation and publishes detections.
    \item \texttt{/vision/detect\_objects}: Performs single-frame inference and returns bounding boxes and distances.
    \item \texttt{/vision/show\_depth\_image}: Displays depth frames for debugging.
\end{itemize}

\textbf{CLIP Classifier Node (\texttt{clip\_classifier})} \\
Performs zero-shot semantic classification on SAM regions.


\begin{itemize}
    \item \texttt{/vision/classify\_all}: Classifies the entire frame.
    \item \texttt{/vision/classify\_bb}: Classifies a user-specified bounding box.
    \item \texttt{/vision/find\_obj}: Given a text label (e.g., ``red cube''), returns the bounding box of the best-matching detected object.
\end{itemize}

\textbf{GraspNet Detector Node (\texttt{graspnet\_detector})} \\
Computes 6-DoF grasp configurations for each SAM detection.


\begin{itemize}
    \item \texttt{/vision/detect\_grasp}: Computes grasp poses for all objects.
    \item \texttt{/vision/detect\_grasp\_bb}: Computes grasp for a specified bounding box.
\end{itemize}

\textbf{Scene Understanding Node (\texttt{scene\_understanding})} \\
Infers spatial relationships such as adjacency, support, containment, and relative distance using SAM detections and geometry.


\begin{itemize}
    \item \texttt{/vision/understand\_scene}: Returns a structured relational graph of the scene.
\end{itemize}

\textbf{Pixel-to-Real Conversion Node (\texttt{pixel\_to\_real})} \\
This utility node converts 2D pixel coordinates into 3D real-world coordinates using the depth camera’s intrinsic matrix.
\begin{itemize}
    \item \texttt{/vision/pixel\_to\_real}: Input: pixel coordinates $(u, v)$; Output: $(x, y, z)$ world coordinates.
\end{itemize}

The node continuously subscribes to \texttt{/camera/depth/image\_raw} to maintain up-to-date depth information for conversions.


% \begin{figure}[h]
% \centering
% \includegraphics[width=\linewidth]{images/vision_service3.png}
% \caption{run_pipeline service flowchart illustrating the sequential execution of perception modules}
% \label{fig:vision_service3}
% \end{figure}

Additionally, \texttt{/vision/run\_pipeline} is a message-based service that triggers the execution of the entire perception pipeline in sequence. When called, it invokes SAM segmentation, CLIP classification (including \texttt{/vision/find\_obj}), GraspNet grasp estimation, scene understanding, and pixel-to-real conversion automatically.



% End session on visual 
\subsubsection{Planning}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{images/ros2_graph_updated.png}
\caption{Updated ROS2 Graph for the Hierarchical Planning System}
\label{fig:planning-ros2-graph}
\end{figure}

Significant progress has been made in improving both the robustness and the intelligence of the planning system. At the low level, we enhanced the \texttt{low\_level\_planner\_executor} node by tightening joint constraints and enforcing Cartesian (linear) path execution rather than free-form trajectories. This change improves motion predictability and safety. 

The \texttt{low\_level\_planner\_executor} node now hosts multiple dedicated action servers, expanding its capability for modular motion control:
\begin{itemize}
\item \texttt{/plan\_cartesian\_relative}: Performs linear movement relative to the current end-effector position.
\item \texttt{/get\_current\_pose} and \texttt{/get\_joint\_angles}: Retrieve the current Cartesian pose and joint configurations, respectively.
\item \texttt{/set\_joint\_angles}: Commands the robot to specific joint positions.
\item \texttt{/plan\_complex\_cartesian\_steps}: Moves the end-effector linearly to a specified pose by decomposing the motion into smaller axis-wise steps.
\end{itemize}

At the next layer, the \texttt{medium\_level\_planner} node was developed to bridge high-level reasoning and low-level execution. This node integrates an AI agent built using LangChain~\cite{langchain-docs}, which dynamically selects and calls the available low-level planner actions as “tools.” It hosts the \texttt{/medium\_level} action server, responsible for receiving and executing subtasks sent from the high-level planner.

The \texttt{high\_level\_planner} node was also implemented with two approaches:
\begin{enumerate}
\item \textbf{LLM-Direct Planning:} An AI agent directly generates a structured sequence of subtasks and corresponding actions to be sent to the \texttt{medium\_level\_planner} for execution.
\item \textbf{Symbolic Planning (PDDL-Based):} The AI agent generates a PDDL problem and domain file from natural language commands. These are then solved by a symbolic planner (FastDownward~\cite{the-fast-downward-planning-system}) to produce an executable plan.
\end{enumerate}
In both cases, the resulting plan, a list of actions, is displayed to the user for confirmation. The user can review, modify, or reject any step using natural language before the plan is passed to the \texttt{medium\_level\_planner} for execution.

Both planner nodes (\texttt{high\_level\_planner} and \texttt{medium\_level\_planner}) are designed to interface with perception modules by accessing services provided by the \texttt{vision} node as additional reasoning tools. Furthermore, safety and emergency handling have been improved by enabling the \\ \texttt{low\_level\_planner\_executor} to subscribe to the \texttt{/emergency} topic, which can immediately halt motion if the \texttt{asr} node detects the emergency keyword.

The system architecture was also updated, as show in Figure~\ref{fig:planning-ros2-graph} such that the \texttt{asr} node now publishes transcribed speech to the \texttt{/transcript} topic upon detecting the activation keyword, simplifying communication flow compared to the previous service-based design.

Finally, both high-level planning approaches were evaluated using user commands of varying complexity and step counts. Metrics such as success rate, execution time, and computational cost (in terms of API requests and token usage) were recorded. The two methods demonstrated comparable performance with a high overall success rate. However, the PDDL-based approach showed slightly lower reliability, likely due to inconsistencies in LLM-generated PDDL syntax. Future work will focus on improving the determinism and correctness of PDDL generation to further increase robustness.
...
\subsubsection{Image Processing}
The development of the Image Processing pipeline has been successfully completed.Core functionalities, including object detection and segmentation using the Segment Anything Model (SAM), object classification with CLIP, grasp synthesis via GraspNet, and scene understanding through spatial reasoning, have been fully implemented and integrated into a modular ROS2 architecture. Each stage of the pipeline has been tested using gazebo simulation, demonstrating reliable performance in identifying tools.

\subsubsection{Speech Processing}
The development of the Automatic Speech Recognition (ASR) module has been successfully completed. Core functionalities, including live audio capture, real-time transcription, and ROS2 publishing, have been fully implemented and ready for validation through both controlled and real-world testing. The system introduces new capabilities such as an \textbf{emergency stop command} for safety assurance and a \textbf{deactivation word} mechanism that allows users to explicitly signal when to send the recognized sentence to the planning node, replacing the previous time-based approach. The ASR system has been successfully integrated with the planning node, enabling seamless and responsive speech-driven robot manipulation. This integration marks the completion of the ASR development phase and establishes a reliable communication interface for future extensions involving multimodal interaction and autonomous task execution.

\subsubsection{Planning}
The current progress is on schedule, with significant advancements made across all levels of the hierarchical planning system. Low-level motion execution has been enhanced for greater stability through tighter joint constraints and enforced Cartesian motion. Both medium- and high-level planners have been implemented, enabling the system to generate, evaluate, and execute multi-step plans using either direct LLM-based reasoning or symbolic PDDL-based planning.

The next phase of development focuses on integrating the planner nodes with the vision services to enable perception-driven planning and decision-making. Concurrently, efforts are underway to refine the planning process itself, improving the robustness of PDDL generation, enhancing inter-node coordination, and ensuring smooth interaction between the language, perception, and motion layers to achieve fully autonomous task execution.
...
In addition to the primary evaluation using the cloud-hosted Gemini-2.5-Flash model, we performed further experiments to assess:  
(1) the performance impact of executing the planner with a locally hosted LLM, and  
(2) the responsiveness of the system during emergency stop scenarios.

\paragraph{Local LLM Performance Comparison}

To reduce cloud dependency and eliminate API rate-limit constraints, we evaluated the planning pipeline using a local language model. We deployed \textbf{GPT-OSS:20b} via \texttt{Ollama} on our computer which is equipped with an NVIDIA RTX 3080 GPU. The same set of 13 task instructions and evaluation methodology were used to ensure comparability with the original Gemini-based experiments.

Table~\ref{tab:local-llm-results} summarizes the results.

\begin{table}[h!]
\centering
\caption{Cloud vs Local LLM Performance Comparison}
\label{tab:local-llm-results}
\begin{tabular}{lcc}
\hline
\textbf{Metric} & \textbf{Cloud} & \textbf{Local} \\
\hline
Execution Time per Step (s) & $7.20 \pm 0.25$ & $17.536 \pm 0.838$ \\
Success Rate (LLM-Direct) & $100.0\%$ & $100.0\%$ \\
Success Rate (PDDL-Based) & $91.0\%$ & $90.7\%$ \\
\hline
\end{tabular}
\end{table}

The locally run LLM achieved nearly identical success rates to the cloud model, indicating that the underlying reasoning quality remains strong even without cloud-scale compute. However, execution time increased significantly, from $M=7.20$ s (Gemini) to $M=17.536$ s (local), with a two-tailed t-test confirming that the difference is \textbf{extremely statistically significant} ($p < 0.0001$).

We believe that future improvements, such as model quantization, GPU inference optimization, or using a smaller local model, could substantially reduce this latency. Nevertheless, the local model provides important practical benefits: improved reliability in unstable network environments, no dependence on external providers, and no exposure to rate limits.

\paragraph{Emergency Stop Latency Evaluation}

To evaluate system safety and responsiveness, we tested how quickly the UR robotic arm stops when the operator speaks the keyword \textit{``emergency''}. During the experiment, the robot was placed in continuous motion while the keyword was issued at random times. 

The results are summarized in Table~\ref{tab:emergency-stop}.

\begin{table}[h!]
\centering
\caption{Emergency Stop Latency Evaluation}
\label{tab:emergency-stop}
\begin{tabular}{lc}
\hline
\textbf{Metric} & \textbf{Value} \\
\hline
Average Stop Time (s) & $1.412$ \\
Standard Deviation (s) & $0.360$ \\
95\% Confidence Interval & $1.412 \pm 0.141$ \\
\hline
\end{tabular}
\end{table}
...
To complement the quantitative and qualitative results presented above, we provide demonstration videos showcasing the integrated performance and important features of the proposed system. 

\subsubsection*{1. Full System Demonstration in Gazebo}
This video illustrates the complete system operating in the Gazebo simulation environment, integrating automatic speech recognition (ASR), the main planning pipeline, and the perception module. The user issues natural language commands referring to specific positions or objects in the scene, and the robot executes the appropriate motion based on information supplied by the vision module.  
\\\textbf{Video:} \href{https://drive.google.com/file/d/1draHgIV_FaXqrEH4p6pD_vMLNN0HLVL3/view?usp=sharing}{\underline{Full System Demonstration}}

\subsubsection*{2. ASR + Planning Pipeline in Simulation and Real Hardware}
This demonstration shows the ASR module working jointly with the planning pipeline in both the Gazebo simulation and the physical UR arm. The two views are overlayed to highlight consistency between the simulated and real environments. In this scenario, only simple movement commands are issued, as the depth-based perception hardware is not yet integrated.  
\\\textbf{Video:} \href{https://drive.google.com/file/d/1yYsGxQITKcc62LWY9pjuGWzi5-gLacgh/view?usp=sharing}{\underline{Simulation + Real Hardware Demonstration}}

\subsubsection*{3. Emergency Stop Functionality on Real Hardware}
This video showcases the safety mechanism implemented in our system. The robot is programmatically commanded to move left and right using ROS~2 action calls. When the user says the emergency keyword \emph{``stop''}, the robot halts immediately and enters an emergency lock state. Subsequent motion commands are ignored until the user explicitly clears the lock by saying \emph{``Okay''}.  
\\\textbf{Video:} \href{https://drive.google.com/file/d/1_-0iNvEOgO9gcwzwICsukVk4yVQje8fW/view?usp=sharing}{\underline{Emergency Stop Demonstration}}

\subsubsection*{4. Plan Revision via Language Commands}
This demonstration highlights the ability of the high-level planner to revise an existing plan based on natural language instructions. The scenario includes both Gazebo simulation and real hardware overlayed. The user initially commands the robot to move upward and then open the gripper. Instead of confirming the plan, the user instructs: \emph{``Swap the action order, execute.''} The system correctly revises the plan, demonstrating contextual memory and dynamic plan adjustment.  
\\\textbf{Video:} \href{https://drive.google.com/file/d/1QjeXoCuzd0tnjiekhAuwajCcLh3JXEpf/view?usp=sharing}{\underline{Plan Revision Demonstration}}
