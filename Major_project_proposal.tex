...
\subsubsection{Planning and Simulation}
...
At the beginning of the semester, we explored simulation in Isaac Sim due to its high visual fidelity and strong integration with ROS2. We were able to import the UR5 robotic arm with a Robotiq 2F-85 gripper, connect it to ROS2 and MoveIt2, and verify planning using both RViz and a custom C++ script. We also integrated an Intel RealSense D435i depth camera and confirmed that perception-enabled planning around obstacles was functional. However, we encountered significant difficulties in simulating the Robotiq 2F-85 gripper: Isaac Sim struggles with closed-loop structures, and despite our efforts, the gripper motion was not simulated correctly. This limitation hindered progress on pick-and-place tasks, which are central to our project.

Following further research into related works, particularly those involving vision-language-action (VLA) models and language-conditioned robotic manipulation, we observed that Gazebo is frequently adopted as the primary simulation environment. In light of this, we shifted our focus from Isaac Sim to Gazebo and began setting up our system there. This decision was motivated both by Gazebo’s widespread use in the literature and its robust integration with ROS2.

Our current Gazebo environment includes a UR5 robotic arm equipped with a Robotiq 2F-85 gripper, a table for placing objects, and a downward-facing depth camera,as shown in Figure~\ref{fig: gazebo-setup}. Using the Gazebo ROS2 plugins, we have verified successful communication with the robot, gripper, and camera, ensuring that sensor data and control commands flow correctly through ROS2. This establishes the foundation for more advanced planning experiments.

We are now working toward enabling pick-and-place tasks in Gazebo, which is known to be a challenging problem due to the complexity of correctly setting up physics properties for object interaction. From our survey of existing solutions, many researchers avoid detailed physics modeling by using specialized plugins that attach an object to the gripper via a fixed joint when a grasp is detected. We tested such a plugin and confirmed that it functions, but in its current state it requires manual specification of the link names to attach, which makes the approach non-dynamic. Our next step is to extend this plugin to support dynamic attachment, thereby better simulating the picking and placing of arbitrary objects.

Additionally, we have reviewed benchmark environments used in VLA research, such as the Colosseum framework~\cite{colosseum}. While these benchmarks provide standardized setups for evaluating language-conditioned manipulation, they are primarily designed for training and testing end-to-end VLA models with curated datasets. Since our approach does not involve training in the same manner, these benchmarks may not be directly applicable. Nevertheless, they remain of interest as potential inspiration for structuring and evaluating our own simulation tasks.

In summary, our simulation pipeline began in Isaac Sim, where ROS2 integration and perception-driven planning were successfully demonstrated, but progress was limited by gripper simulation issues. We have since transitioned to Gazebo, where the environment is nearly complete with robot, gripper, camera, and table setup. Current efforts are focused on enabling realistic pick-and-place interactions through dynamic attachment, while also considering insights from benchmark simulation frameworks for future evaluation.
