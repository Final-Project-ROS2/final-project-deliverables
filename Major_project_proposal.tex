...
An acrylic plate was also installed on the robot base to increase stability and expand the base coverage so that the plate fully supports and protects the camera frame. This plate enlarges the stable support area for the mounted sensor assembly, reduces vibration, and helps preserve the calibrated camera pose throughout the experiment. Together with the rigid mount and cable management, these mechanical improvements ensure a more stable, repeatable, and safer hardware setup for real-world testing.
...
Mechanical integration has been further refined through the implementation of a structured cable management solution. A custom 3D-printed cable mount has been designed and installed on the camera stand to secure and guide the cabling. This prevents unwanted cable movement and ensures that wiring does not interfere with the UR arm’s range of motion. In addition, an acrylic plate was installed on the robot base to increase stability and to fully cover the camera frame, providing a larger and more stable support surface for the mounted sensors. These improvements enhance system robustness, operational safety, and overall deployment efficiency.
\newpage
