...
At the \textbf{high level}, natural language instructions are translated into structured task plans. Two approaches are supported. In the neural approach, a large language model (LLM) directly generates a sequence of subtasks, providing flexibility but limited guarantees on correctness. In the neuro-symbolic approach, instructions are converted into a symbolic representation using the Planning Domain Definition Language (PDDL), which is solved using the Fast Downward planner to produce a logically valid plan.

For the neuro-symbolic approach, we evaluate two PDDL problem-generation strategies. In the \textbf{Direct Generation} setting, the LLM generates the full PDDL problem text from the textual task description. In the \textbf{IR-Based Generation} setting, the LLM first generates a structured intermediate representation (IR) in JSON format, then a deterministic conversion step produces the final PDDL problem file.

The JSON IR contains three core elements: \texttt{objects}, \texttt{init} predicates, and \texttt{goals}. This reduces the model's burden of producing strict symbolic syntax directly while preserving the semantic content required for planning.
...
As illustrated in Figure~\ref{fig:pddl-pipeline}, the structured IR is deterministically transformed into a valid PDDL problem file using Jinja2 templating. The domain file remains fixed and manually authored, ensuring strict control over predicates and actions. This two-stage pipeline improves reliability by separating semantic generation (LLM) from formal syntax rendering (template).

The \textbf{medium-level planner} bridges high-level plans and execution by decomposing subtasks into sequences of callable robot capabilities. It interacts with the low-level system through action servers, treating motion primitives as tools that can be dynamically composed based on task requirements and system state.

At the \textbf{low level}, the planner executes motion commands using constrained path and trajectory planning to ensure smooth and safe operation. It also provides feedback such as execution status and robot state, enabling monitoring and error handling across the hierarchy.

Overall, this hierarchical and hybrid planning framework enables reliable transformation of natural language instructions into executable robot behavior, while maintaining flexibility, safety, and modularity.
