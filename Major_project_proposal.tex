...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{images/rviz_planning.png}
    \caption{UR5 with Robotiq 2F-85 gripper controlled via MoveIt2 in RViz}
    \label{fig: rviz-planning}
\end{figure}

The primary goal this semester is to establish a complete planning and simulation pipeline for the UR5 robotic arm with a Robotiq 2F-85 gripper. The setup is designed to reflect the physical hardware that will be deployed in future experiments, while enabling safe and flexible testing in simulation.

We first ensured reliable communication between Isaac Sim, running on the Windows host machine, and ROS2 Humble, running inside Ubuntu 22.04 on WSL2. On top of this, we integrated MoveIt2 to handle motion planning using planners from the OMPL library. A custom URDF model of the UR5 with the Robotiq 2F-85 gripper was created, and a corresponding MoveIt2 configuration package was generated. This configuration was successfully imported into Isaac Sim, allowing ROS2 and MoveIt2 to control the robot.

The connection was validated in two ways: (1) interactively, by setting target poses through RViz (Figure~\ref{fig: rviz-planning}), and (2) programmatically, using a simple C++ script to specify and execute target positions. Both approaches confirmed correct integration between Isaac Sim, ROS2, and MoveIt2.

To enable obstacle-aware planning, we incorporated perception into the pipeline by adding an Intel RealSense D435i depth camera to the Isaac Sim environment. An action graph was created to publish point clouds to \texttt{/camera\_1/points} and RGB frames to \texttt{/rgb}, with the camera’s static transform published to \texttt{/tf\_static}. Visualization in RViz confirmed successful reception of point clouds, and we demonstrated planning around obstacles created in Isaac Sim using this data.

Despite these advances, the Robotiq 2F-85 gripper remains problematic. Isaac Sim struggles with closed-loop structures, and the gripper’s motion is not yet simulated correctly. This requires additional setup, which we are currently investigating.  

As a contingency, we set up a parallel simulation environment in Gazebo, running natively on Ubuntu 22.04 within WSL2. The UR5 with the Robotiq 2F-85 gripper was successfully configured in Gazebo, and communication with ROS2 and MoveIt2 was verified. Although Gazebo offers a simpler and more stable integration path, its lower realism, particularly in handling complex collision geometries such as engineering tools, makes it less suitable for the final system. Our strategy is therefore to prioritize Isaac Sim while keeping Gazebo as a reliable fallback.

In summary, the low-level planning and simulation stack is now operational across both Isaac Sim and Gazebo. We have demonstrated integration of ROS2, MoveIt2, and perception-enabled planning, and verified collision-free trajectories in a simulated environment. Remaining tasks include resolving the gripper simulation challenges in Isaac Sim and refining the setup for pick-and-place and handover scenarios.
