...
The speech module enables natural language interaction between the user and the robotic system through an Automatic Speech Recognition (ASR) pipeline. It is designed to convert spoken commands into structured textual instructions that can be processed by the planning subsystem.

The module consists of three main components: audio input capture, speech-to-text processing, and command validation. A microphone continuously captures audio signals, which are then processed by the ASR engine to generate text transcripts in real time. These transcripts are published to a ROS2 topic, allowing downstream modules to subscribe and react in an event-driven manner.

To ensure reliable execution and prevent unintended actions, the system employs a keyword-based gating mechanism. A command is only forwarded to the planning module when it contains the predefined activation keyword “EXECUTE.” This design ensures that partial or unintended speech inputs do not trigger robot actions.

In addition, the system incorporates a safety mechanism through an emergency keyword. When the word “STOP” is detected, an emergency signal is immediately published to a dedicated ROS2 topic, which is monitored by the low-level controller. This allows the robot to halt all ongoing motions instantly. The system remains in a halted state until a recovery keyword (“OKAY”) is received.

The speech module is designed to be modular and extensible, allowing integration with more advanced natural language understanding components or multimodal interaction systems in future developments.
\subsection{Planning}
