...
The hardware subsystem consists of a UR7 robotic manipulator integrated with a depth camera mounted at the base of the robot. The camera is rigidly fixed relative to the robot base, establishing a stable and consistent spatial relationship between the sensing and manipulation components.

A fixed transformation between the camera frame and the robot base frame is defined to enable accurate perception-to-action mapping. This transformation supports reliable conversion from visual observations to robot-executable coordinates, eliminating the need for repeated calibration across operational sessions.

The experimental workspace is configured with a controlled visual background to improve perception robustness. An acrylic plate is installed within the camera’s field of view to provide a uniform and consistent background, reducing visual noise and improving the reliability of object detection and segmentation.

Mechanical integration is further enhanced through a structured cable management system. A custom-designed mount secures and guides all cables along the camera support structure, preventing cable-induced disturbances and ensuring that wiring does not interfere with the robot’s workspace or motion range.

To improve overall structural stability, the camera mounting system is rigidly fixed to the robot base, minimizing vibration and ensuring consistent sensor positioning during robot motion.

Overall, the hardware configuration provides a rigid, repeatable, and interference-free platform that supports reliable perception, safe robot operation, and seamless integration with the planning and control subsystems.
\newpage
...
Despite establishing a stable and reproducible hardware execution environment, several limitations still exist. In particular, motion from the UR7e causes instability in the camera mount, leading to vibrations during operation and reducing the consistency of visual pose estimation. Additionally, the acrylic plate mounted on the base lacks sufficient rigidity, bending under load and failing to provide proper support for the camera frame. These mechanical shortcomings affect repeatability, introduce calibration drift, and need to be resolved for reliable real-world, perception-driven performance.

Future work should focus on improving the mechanical stability and robustness of the system. The camera mounting structure can be redesigned using more rigid materials such as aluminum or steel, along with proper brackets and vibration-damping components to minimize movement. Replacing the current acrylic base with a stiffer platform would further enhance structural integrity. In addition, calibration refinement techniques, such as periodic or online recalibration, can help mitigate drift caused by residual instability. Exploring sensor fusion approaches to complement vision-based perception may also improve robustness. Finally, optimizing motion planning to reduce abrupt movements of the robotic arm could further decrease vibrations, leading to more stable perception and improved repeatability in real-world tasks.
