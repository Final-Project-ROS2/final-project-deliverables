...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{2.5cm}p{3cm}p{3.5cm}@{}}
        \toprule
        \textbf{Camera Height} & \textbf{Intrinsic Only (RMSE)} & \textbf{Intrinsic + Empirical (RMSE)} \\
        \midrule
...
The vision subsystem of this project successfully integrated a complex pipeline of state-of-the-art models, including the Segment Anything Model (SAM), CLIP, and GraspNet, utilizing Oriented Bounding Boxes (OBB) to guide the URARM's manipulation tasks. Navigating the integration of these models provided profound insights into both applied computer vision and academic research methodologies.

Looking back at the development of the vision pipeline, the learning curve extended far beyond simply writing code and connecting APIs. Reflecting on the key takeaways from this project and considering potential improvements for future iterations, the insights fall into three main areas:

\paragraph{From Black-Box Implementation to Mathematical Understanding}
Initially, the focus was heavily on model implementation. However, the most significant technical growth resulted from analyzing the underlying mathematics operating behind the scenes. Transitioning from treating models such as SAM and GraspNet as ``black boxes'' to comprehending their core image processing algorithms allowed for more effective troubleshooting and optimization. We learned that robust engineering in computer vision requires understanding \textit{why} a model behaves a certain way at a mathematical level, rather than merely knowing \textit{how} to execute its functions.

\paragraph{Navigating Model Limitations and Environmental Constraints}
A major takeaway was the necessity of critically evaluating the specific use cases and inherent limitations of each model. Theoretical capabilities frequently conflict with real-world constraints. For instance, we gained firsthand experience regarding how lighting variations and specific object geometries drastically impact model accuracy. If initiating the project today, we would invest significantly more time during the early stages to test the models against extreme environmental variables (such as harsh lighting or varied object models). This proactive approach would facilitate the development of a more robust, fault-tolerant pipeline from the outset, rather than addressing edge cases retroactively.

\paragraph{Mastering the Research Methodology}
Perhaps the most valuable outcome of this project was experiencing the complete lifecycle of academic research. This endeavor reinforced rigorous research methodologies from formulating a structured thought process and transitioning a raw idea into a formal experiment, to ultimately writing and submitting a research paper. We learned to systematically document issues, propose and implement solutions, and track performance improvements over time. The journey from conceptualization to drafting the final manuscript has fundamentally refined our approach to problem-solving, demonstrating that comprehensive documentation and structured iteration are just as critical as the underlying code.
