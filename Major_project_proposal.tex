...
% \begin{figure}[h]
% \centering
% \includegraphics[width=\linewidth]{images/planning_architecture_updated.png}
...
At the next layer, the \texttt{medium\_level\_planner} node was developed to bridge high-level reasoning and low-level execution. This node integrates an AI agent built using LangChain, which dynamically selects and calls the available low-level planner actions as “tools.” It hosts the \texttt{/medium\_level} action server, responsible for receiving and executing subtasks sent from the high-level planner.
...
\item \textbf{Symbolic Planning (PDDL-Based):} The AI agent generates a PDDL problem and domain file from natural language commands. These are then solved by a symbolic planner (FastDownward) to produce an executable plan.
\end{enumerate}
