...
Voice-based interaction has played a central role in enabling natural communication between humans and robots. Two key approaches that have been widely studied are \textbf{Voice Control Systems (VCS)} and \textbf{Automatic Speech Recognition (ASR)}, both of which contribute to making robot manipulation more intuitive and user-friendly.  

\textbf{Traditional Voice Control Systems (VCS).}  
VCS applications have demonstrated the potential of speech as a hands-free, natural interface in multiple domains~\cite{vcs}:  
\begin{itemize}
    \item \textbf{Early Childhood Education:} Speech-activated robotics systems enable children to communicate with robots through simple voice commands, fostering intuitive interaction.  
    \item \textbf{Smart Home Automation:} By integrating IoT, Fog, and Cloud architectures with speech-to-text and text-to-speech systems, users can control lights and appliances through voice interfaces.  
    \item \textbf{Assistance for Individuals with Disabilities:} Low-cost voice-based home automation solutions allow people with paraplegia or quadriplegia to control adjustable beds or other devices with simple commands.  
    \item \textbf{Healthcare Applications:} Voice recognition on mobile devices helps medical staff record workflow milestones efficiently, reducing the burden of manual data entry.  
    \item \textbf{Traffic Management:} Voice-controlled systems can adjust traffic signals based on simple commands such as ``red,'' ``green,'' or ``yellow.''  
\end{itemize}  

Despite their wide use, traditional VCS are typically limited to \textit{predefined tasks}. They struggle with grounding fine-grained concepts (e.g., color, material, or spatial relations), algorithmic reasoning (e.g., counting, comparing), and often function as black-box systems that require large datasets for training and lack interpretability~\cite{vcs}. Security and data privacy also remain ongoing concerns.  

\textbf{Automatic Speech Recognition (ASR) and Multimodal ASR.}  
To overcome these limitations, modern systems have moved toward \textbf{Automatic Speech Recognition (ASR)}, particularly \textbf{multimodal ASR}, which integrates both speech and visual context to improve accuracy in real-world, noisy environments~\cite{Chang2023}. This approach is better aligned with the conditions faced by physical robots that must interpret spoken rather than written instructions.  

Key purposes and mechanisms of multimodal ASR include:  
\begin{itemize}
    \item \textbf{Addressing Input Gaps:} While many benchmarks assume text-based instructions, robots deployed in practice must process spoken commands. Multimodal ASR bridges this gap by converting speech into text~\cite{Chang2023}.  
    \item \textbf{Reducing Recognition Errors:} Unlike unimodal ASR, which may misinterpret unclear words, multimodal ASR leverages accompanying visual information to resolve ambiguities~\cite{Chang2023}.  
    \item \textbf{Example Scenario:} If a robot hears ``Put the egg in the \ldots'' but the final word is noisy, unimodal ASR might incorrectly guess ``fridge.'' Multimodal ASR, however, can use visual context (e.g., spotting a microwave nearby) to correctly infer ``microwave''~\cite{Chang2023}.  
\end{itemize}  

\textbf{Architecture of Multimodal ASR:}  
\begin{itemize}
    \item \textbf{Speech Encoding:} Spoken instructions are processed by a speech encoder (e.g., wav2vec 2.0) to extract meaningful features~\cite{Chang2023}.  
    \item \textbf{Visual Encoding:} The robot’s current visual observation is processed by a visual encoder (e.g., CLIP-ViT) into a feature vector~\cite{Chang2023}.  
    \item \textbf{Joint Decoding:} A Transformer-based decoder attends to both the speech and visual encodings, combining them to predict an accurate textual transcription of the spoken command~\cite{Chang2023}.  
\end{itemize}  

Together, VCS and multimodal ASR represent complementary solutions for enabling natural language robot manipulation. VCS laid the foundation for voice-driven interfaces in defined contexts, while multimodal ASR extends these capabilities to \textit{open, context-rich environments}, where accurate and adaptive interpretation of spoken instructions is crucial~\cite{vcs},\cite{Chang2023}.
...
Our exploration has focused on leveraging foundation models such as the Segment Anything Model (SAM) for segmentation and CLIPs for classification, while ensuring modularity that allows integration with grasping algorithm (e.g., GraspNet) and higher-level reasoning (e.g. Vision-Language Models for scene understanding) \\

In this report, we implemented and tested the first two stages of our vision pipeline: object detection/segementation and object classification 

\begin{itemize}

\item Object Segmentation
We used the SAM model to segment objects in tool-rich images. SAM successfully generated fine-grained masks and bounding boxes, producing over 50 detected regions per image. However, elongated objects such as pliers were often split into multiple masks. To address this, we implemented a mask merging strategy based on bounding box overlap and mask adjacency. This reduced redundant detections and allowed us to consolidate fragmented masks into a single object representation.
[img]
...
We will extend the refinement module with embedding similarity checks to further improve mask consolidation. This will prepare the data flow for the third stage of the pipeline which is grasp synthesis.
...
Once the PDDL code is generated, We use the Fast Downward planner to test whether the resulting domain and problem files are syntactically correct and solvable.
